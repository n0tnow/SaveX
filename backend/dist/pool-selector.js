...
function categorizePool(pool, tokenMap) {
    ...
    // Major pair kontrolü
    const isMajor = (issuer) => MAJOR_TOKENS[issuer] !== undefined;
    ...
    const isStablecoin = (code) => STABLECOIN_KEYWORDS.some(kw => code.includes(kw));
...
function calculatePoolScore(pool, tokenMap, maxShares, maxPopularity) {
    const reserves = pool.reserves || [];
    const asset1 = parseAsset(reserves[0]?.asset || '');
    const asset2 = parseAsset(reserves[1]?.asset || '');
...
// Top 500 pool seç
export async function selectTop500Pools() {
    ...
    const maxPopularity = Math.max(...tokensData.tokens.map((t) => t.pools));
...
// Sonuçları kaydet
export function saveSelectedPools(result) {
