...
async function invokeContractViaRPC(contractId, functionName, args, sourceKeypair) {
    ...
    const tx = new StellarSdk.TransactionBuilder(account, {
        ...
        .addOperation(contract.call(functionName, ...args.map(arg => StellarSdk.nativeToScVal(arg))))
        .setTimeout(30)
        .build();
...
async function createTestnetToken(sourceKeypair, code, decimals, initialAmount) {
...
async function getOrCreatePool(sourceKeypair, tokenA, tokenB) {
    ...
    try {
        ...
        const tx = new StellarSdk.TransactionBuilder(account, {
            ...
            .addOperation(factory.call('get_pair', StellarSdk.nativeToScVal(tokenA), StellarSdk.nativeToScVal(tokenB)))
            .setTimeout(30)
            .build();
        tx.sign(sourceKeypair);
        const xdr = tx.toXDR();
    ...
    try {
        ...
        const tx = new StellarSdk.TransactionBuilder(account, {
            ...
            .addOperation(factory.call('create_pair', StellarSdk.nativeToScVal(tokenA), StellarSdk.nativeToScVal(tokenB)))
            .setTimeout(30)
            .build();
        tx.sign(sourceKeypair);
        const xdr = tx.toXDR();
...
async function addOrUpdateLiquidity(sourceKeypair, tokenA, tokenB, amountA, amountB, minAmountA = '0', minAmountB = '0') {
    ...
    try {
        const tx = new StellarSdk.TransactionBuilder(account, {
            ...
            .addOperation(router.call('add_liquidity', StellarSdk.nativeToScVal(tokenA), StellarSdk.nativeToScVal(tokenB), StellarSdk.nativeToScVal(amountA), StellarSdk.nativeToScVal(amountB), StellarSdk.nativeToScVal(minAmountA), StellarSdk.nativeToScVal(minAmountB), StellarSdk.nativeToScVal(sourceKeypair.publicKey()), StellarSdk.nativeToScVal(deadline)))
            .setTimeout(30)
            .build();
        tx.sign(sourceKeypair);
        const xdr = tx.toXDR();
...
export async function syncMainnetPoolToTestnet(sourceSecret, pairName, updateOnly = false) {
    ...
    if (existingPool && updateOnly) {
        ...
        const nativeReserve = reserves.find((r) => r.asset === 'native');
        const tokenReserve = reserves.find((r) => r.asset !== 'native');
        if (!nativeReserve || !tokenReserve) {
            throw new Error('Invalid pool reserves');
        }
        await addOrUpdateLiquidity(sourceKeypair, existingPool.tokenAAddress, existingPool.tokenBAddress, nativeReserve.amount, tokenReserve.amount);
        // State'i güncelle
    ...
    const tokenAAddress = await createTestnetToken(sourceKeypair, tokenA.code, tokenA.decimals, tokenA.amount);
    const tokenBAddress = await createTestnetToken(sourceKeypair, tokenB.code, tokenB.decimals, tokenB.amount);
    // Pool oluştur veya al
    const poolAddress = await getOrCreatePool(sourceKeypair, tokenAAddress, tokenBAddress);
    // Likidite ekle
    await addOrUpdateLiquidity(sourceKeypair, tokenAAddress, tokenBAddress, tokenA.amount, tokenB.amount);
    // State'i kaydet
    const poolState = {
