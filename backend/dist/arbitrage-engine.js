...
function detectDirectArbitrage(pools, externalPrices) {
    const opportunities = [];
...
function findTriangularPaths(pools) {
    const paths = [];
    const tokenGraph = {};
    // Graph oluştur
    for (const pool of pools) {
...
function detectTriangularArbitrage(pools, externalPrices) {
    const opportunities = [];
...
export async function detectAllArbitrageOpportunities() {
    ...
    const pools = poolsData.selectedPools;
    ...
    const externalPrices = pricesData.prices;
...
// Sonuçları kaydet
export function saveArbitrageOpportunities(opportunities) {
