...
async function discoverAllMainnetPools() {
    try {
...
async function fetchMainnetPool(pair) {
    try {
        ...
        for (const pool of pools) {
            const reserves = pool.reserves || [];
            const hasNative = reserves.some((r) => r.asset === 'native');
            const hasTargetAsset = reserves.some((r) => {
                if (pair.assetB.type === 'native')
                    return r.asset === 'native';
        ...
        const nativePools = pools.filter((p) => p.reserves?.some((r) => r.asset === 'native'));
...
async function fetchAllMainnetPools() {
    const results = {};
...
async function syncToTestnet(snapshot) {
...
export async function startSyncService(intervalMinutes = 5) {
