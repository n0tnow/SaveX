 ...
 */
async function queryContract(contractId: string, functionName: string, args: any[] = []): Promise<any> {
 ...
 * Fetch pool reserves
 */
async function fetchPoolData(pool: typeof KNOWN_POOLS[0]): Promise<PoolData> {
  try {
 ...
 */
export async function getPoolsData(): Promise<PoolData[]> {
  const now = Date.now();
 ...
 */
export async function getPoolById(address: string): Promise<PoolData | null> {
