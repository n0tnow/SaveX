...
interface DiscoveredToken {
  ...
  issuer: string;
  type: string;
  pools: number; // Kaç pool'da kullanılıyor
}

interface PoolAnalytics {
  totalPools: number;
  nativePools: number; // XLM içeren
  tokenPools: number; // Token-token çiftleri
  totalTokens: number;
  ...
  poolsByCategory: {
    major: string[]; // XLM/USDC gibi büyük pool'lar
    stablecoin: string[]; // Stablecoin çiftleri
    defi: string[]; // DeFi token'ları
    other: string[];
  };
}

// Tüm liquidity pool'ları çek (pagination ile)
export async function fetchAllMainnetPools(): Promise<MainnetPool[]> {
  console.log('[Discovery] Fetching ALL mainnet liquidity pools...');
  console.log('[Discovery] This may take a few minutes...\n');

  const allPools: MainnetPool[] = [];
...
// Pool'ları analiz et
export function analyzeMainnetPools(pools: MainnetPool[]): PoolAnalytics {
  console.log('\n[Analysis] Analyzing pools...');

  const tokens: Record<string, DiscoveredToken> = {};
  ...
  for (const pool of pools) {
    ...
    if (hasNative) {
      ...
      // Stablecoin pairs
      const isStablecoin = (code: string) =>
        code.includes('USD') || code.includes('EUR') ||
...
// Pool'ları kaydet
export function savePoolsToFile(pools: MainnetPool[], filename: string = 'all_mainnet_pools.json') {
...
export function saveAnalyticsReport(analytics: PoolAnalytics) {
...
export function saveTokenList(analytics: PoolAnalytics) {
...
// Konsol raporu
export function printAnalyticsReport(analytics: PoolAnalytics) {
  console.log('\n' + '='.repeat(70));
