...
app.get('/api/pools/:id', (req, res) => {
    ...
    const pool = data.selectedPools.find((p: any) => p.poolId === req.params.id);
...
app.get('/api/arbitrage', (req, res) => {
    ...
    if (minProfit) {
        ...
        opportunities = opportunities.filter((o: any) => o.profitPercent >= minProfitNum);
    }

    // Filter by confidence
    if (confidence && typeof confidence === 'string') {
        opportunities = opportunities.filter((o: any) => o.confidence === confidence);
    }
...
app.get('/api/tokens', (req, res) => {
    ...
    if (minPools) {
        const minPoolsNum = parseInt(minPools as string);
        tokens = tokens.filter((t: any) => t.pools >= minPoolsNum);
    }
